...
static ENGINE_ERROR_CODE initalize_configuration(struct cuckoo_engine *se,
                                                 const char *cfg_str);
...
static void set_vbucket_state(struct cuckoo_engine *e,
...
static vbucket_state_t get_vbucket_state(struct cuckoo_engine *e,
                                         uint16_t vbid) {
    union vbucket_info_adapter vi;
    vi.c = e->vbucket_infos[vbid];
    return vi.v.state;
...
static bool handled_vbucket(struct cuckoo_engine *e, uint16_t vbid) {
...
ENGINE_ERROR_CODE create_instance(uint64_t interface,
   ...
   struct cuckoo_engine *engine = malloc(sizeof(*engine));
   if (engine == NULL) {
      return ENGINE_ENOMEM;
   }

   struct cuckoo_engine cuckoo_engine = {
      .engine = {
         .interface = {
            .interface = 1
         },
   ...
   *engine = cuckoo_engine;
   engine->tap_connections.clients = calloc(cuckoo_engine.tap_connections.size, sizeof(void*));
...
static inline struct cuckoo_engine* get_handle(ENGINE_HANDLE* handle) {
   return (struct cuckoo_engine*)handle;
}
...
static ENGINE_ERROR_CODE default_initialize(ENGINE_HANDLE* handle,
                                            const char* config_str) {
   struct cuckoo_engine* se = get_handle(handle);
...
static ENGINE_ERROR_CODE default_item_allocate(ENGINE_HANDLE* handle,
                                               ...
                                               const int flags,
                                               const rel_time_t exptime) {
   struct cuckoo_engine* engine = get_handle(handle);
...
static void stats_vbucket(struct cuckoo_engine *e,
                          ADD_STAT add_stat,
                          const void *cookie) {
...
static bool set_vbucket(struct cuckoo_engine *e,
                        const void* cookie,
...
static bool get_vbucket(struct cuckoo_engine *e,
                        const void* cookie,
...
static bool rm_vbucket(struct cuckoo_engine *e,
                       const void *cookie,
...
static bool scrub_cmd(struct cuckoo_engine *e,
                      const void *cookie,
                      protocol_binary_request_header *request,
                      ADD_RESPONSE response) {
...
static bool touch(struct cuckoo_engine *e, const void *cookie,
                  protocol_binary_request_header *request,
                  ADD_RESPONSE response) {
...
static ENGINE_ERROR_CODE default_unknown_command(ENGINE_HANDLE* handle,
                                                 ...
                                                 ADD_RESPONSE response)
{
    struct cuckoo_engine* e = get_handle(handle);
    bool sent;
...
static void default_handle_disconnect(const void *cookie,
                                      ...
                                      const void *event_data,
                                      const void *cb_data) {
    struct cuckoo_engine *engine = (struct cuckoo_engine*)cb_data;
    pthread_mutex_lock(&engine->tap_connections.lock);
    int ii;
    for (ii = 0; ii < engine->tap_connections.size; ++ii) {
